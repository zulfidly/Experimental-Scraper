...
<template>
  <div
    ...
    <table class="relative w-[inherit] h-full bg-[var(--color-background-mute)]">
      ...
      <tbody>
        <tr v-for="(x, ind) in records" ref="listTR" :key="'row' + ind"
          class="[&>*]:mx-auto [&>*]:even:bg-[var(--color-background-soft)] [&>*]:p-0.5 [&>*]:lg:p-2 [&>*]:text-center [&>*]:font-light [&>*]:lg:font-medium">
          <td v-if="isMobileAndLandscape || isDesktop" class="table-cell">{{ new Intl.DateTimeFormat('en-GB').format(new
            Date(x.fields.Date)) }}</td>
          <td v-else class="table-cell">{{ new Intl.DateTimeFormat('en-GB').format(new Date(x.fields.Date)).slice(0, 5) }}
          </td>
