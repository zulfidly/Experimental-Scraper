...
<template>  
  <div :style="getHeight" class="relative w-full flex flex-col justify-evenly items-center overflow-clip lg:border lg:rounded-xl lg:shadow-md transition-all duration-700">
    <p class="sm:hidden lg:block p-3 fixed lg:absolute left-0 bottom-0 text-sm italic font-thin">* figures shown in MYR</p>
    <p class="sm:hidden lg:block text-center text-md lg:text-lg font-normal lg:font-bold tracking-wide">
    ...
    <ColorModeSwitch @click="toggleDark()" class="sm:hidden lg:block mr-4 place-self-end" />

    <div class="flex flex-col relative lg:px-6 w-[inherit] max-h-[70svh] sm:max-h-[99svh] lg:max-h-[65svh] overflow-y-scroll border-[var(--color-border)]">  
      <table class="relative w-[inherit] h-full bg-[var(--color-background-mute)]">
        <thead class="sticky top-0 bg-[var(--table-header)]">
          <tr class="[&>*]:mx-auto [&>*]:lg:p-2 [&>*]:text-md [&>*]:font-light [&>*]:lg:font-bold [&>*]:text-center">
            ...
            <th class="hidden md:table-cell">Time<br>Read (GMT+8)</th>
    ...
    <section class="border border-lime-100 sm:hidden lg:block place-self-end mt-4 lg:mr-6">
